<template>
  <div class="center" >
    ...
    <table>
      <thead>
        <tr  >
          <th class="budget__month" v-for="(item, index) in monthes" :key="index">{{item}}</th>
        </tr>
      </thead>
      <tbody>
...
export default {
  data() {
    return {
      monthes:['Статья','Январь','Февраль','Март','Апрель','Май','Июнь','Июль','Август','Сентябрь','Октябрь','Ноябрь','Декабрь','Итого'],
      incomItems: [
